    ...
    public partial class FormMain : Form
    {
        ...
        public FormMain()
        {
            ...
            for (int i = 0; i < 8; i++)
            {
                ...
                loadMesh.SetDefaultMaterial();
                var loadInstanceModel = new InstancedModel(loadMesh);
#else
                Model3D toteWhiteModel = new Model3D("Assets/totewhite.dae");
                toteWhiteModel.GetMesh(0).CenterAtOrigin();
                toteWhiteModel.GetMesh(0).Normalize();
                toteWhiteModel.GetMesh(0).SetColor(colors[i]);
                Vector3 dims = dimensions[i];
                dims.Y *= 1.3f;
                // In this model the y is up, we want z up so rotate 90 degrees around x
                toteWhiteModel.GetMesh(0).TransformVertices(Vector3.Zero, new Vector3(1, 0, 0), 0.25f, dimensions[i]);
                toteWhiteModel.GetMesh(0).TranslateToFloor();
                toteWhiteModel.GetMesh(0).TransformVertices(new Vector3(0, 0, 0.05f));
                var loadInstanceModel = new InstancedModel(toteWhiteModel.GetMesh(0));
#endif
            ...
            for (int i = 0; i < 8; i++)
            {
                ...
                pMesh.SetDefaultMaterial();

                var pInstanceModel = new InstancedModel(pMesh);
            ...
            emptyPalletMesh.SetDefaultMaterial();
            var palletInstanceModel = new InstancedModel(emptyPalletMesh);
        ...
        {
            modelContainer.Update((ulong)(modelContainer.GetPrecision() * (float)(ersRunControl1.StepSize / 60.0f)));
            timeCounter++;
            if (timeCounter % 15 == 0)
            {
                timeCounter = 0;
        ...
        private void Render2D(object? sender, RenderEventArgs e)
        {
            ...
            Vector3 gridColor = new Vector3(0.75f, 0.68f, 0.65f);
            e.Context.DrawInfiniteGrid2D(gridColor, 1.0f, 0.1f, 64.0f);

            // Nice light sky blue background
            e.Context.SetBackgroundColor(new Vector3(0.7f, 0.85f, 1.0f));
            ...
            Vector4 beltColor = new Vector4(0.9f, 0.9f, 0.9f, 1.0f);
            while (conveyorView.Next())
            {
                ...
                switch (segment.Type)
                {
                    case PathSegmentType.Straight:
                        ...
                        e.Context.DrawRect2D(beltCenter, beltDims, angle, beltColor);
                        break;
                    ...
                    case PathSegmentType.Helical:
                        ...
                        e.Context.DrawArc2D(
                            segment.Center.XY(), segment.Radius, ConveyorBuilder.beltWidth, segment.BeginAngle, endAngle, beltColor, 20);
                        break;
                }
            }
            ...
            while (bayView.Next())
            {
                ...
                e.Context.DrawText2D(entity.GetName(), new Vector2(pos.X, pos.Y), 1.0f);
            }
            bayView.Dispose();
        ...
        {
            Simulator firstSimulator = modelContainer.GetSimulator(0);
            firstSimulator.EnterSubModel();
            SubModel subModel = SubModel.GetSubModel();
            ...
            while (viewLoads.Next())
            {
                ...
                transportableInstanceModel.PushInstance(transform.Value.GetGlobalPosition3D(), transform.Value.GetGlobalRotation3D());
            }
            viewLoads.Dispose();

            for (int i = 0; i < loadInstanceModels.Count(); i++)
            {
                var instanceModel = loadInstanceModels[i];
                e.Context.DrawInstancedModel3D(instanceModel);
                instanceModel.Clear();
            }
            ...
            e.Context.DrawText3D("Depalletizer", new Vector3(0, 0, 18), normal, up, scale, color);
            e.Context.DrawText3D("Sorter", new Vector3(40, 8, 7), normal, up, scale, color);
            e.Context.DrawText3D("Picking Stations", new Vector3(40, -10, 2), normal, up, scale, color);
            e.Context.DrawText3D("Highbay Storage", new Vector3(5, -51, 30), normal, up, scale, color);
            e.Context.DrawText3D("Inbound", new Vector3(-80, 80, 30), normal, up, scale, color);

            // SubModel.GetSubModel().PrintGCStats();
            firstSimulator.ExitSubModel();
            e.Context.DrawInfiniteGrid3D(new Vector3(0.5f, 0.5f, 0.5f), 1.0f, 64.0f);
        }
        ...
        private void ScriptToolStripMenuItem_Click(object sender, EventArgs e) =>
            new FormScriptEditor().Show();

        private void ReloadPythonModules()
        {
            // Note: This should be different when publishing!!! BE CAREFUL. Only for quick hotreloading during development.
        ...
        private void RestartModel()
        {
            ...
            //var pInstanceModel    = new InstancedModel(convMesh);
