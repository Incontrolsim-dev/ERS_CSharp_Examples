import ERS
class Merge(ERS.ScriptBehavior):
    def __init__(self, entity, num_inputs=1, throughput_time=1_000_000):
        super().__init__(entity)
        self.throughput_time = throughput_time
        ERS.add_atom_behavior(entity)
        ERS.set_num_inputs(entity, num_inputs)
...
class Split(ERS.ScriptBehavior):
    def __init__(self, entity, routing, num_outputs=1):
        super().__init__(entity)
        print(f"Creating split with {num_outputs} outputs")
        self.throughput_time = 1_000_000
        self.routing = routing
        ERS.add_atom_behavior(entity)
        ERS.set_num_inputs(entity, 1)
        ERS.set_num_outputs(entity, num_outputs)
    
    def route(self, transportable_entity):
        return 0
    ...
    def on_entered(self, new_child):
        ...
        # Just always send
        #print(f"{self.entity.name} is connected to output: {ERS.is_connected(self.entity, 0)}")
        output_idx = self.route(new_child)
        if ERS.is_connected(self.entity, 0):
            #print(f"{self.entity.name} is sending {new_child.name}")
            ERS.schedule_local_event(self.throughput_time, lambda: ERS.channel_output_send(self.entity, output_idx, new_child))
